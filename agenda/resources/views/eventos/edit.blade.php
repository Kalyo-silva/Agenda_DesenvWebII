<x-app-layout>
    <x-slot name="header">
        <div class='flex items-center'>
            <div class='w-1/2'>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Editar Evento') }}</h2>
            </div>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('eventos.update', $evento->id) }}" id="formEvento">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" name="data_ini" id="data_ini" value="{{ $evento->data_ini->format('Y-m-d\TH:i') }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" name="data_fim" id="data_fim" value="{{ $evento->data_fim->format('Y-m-d\TH:i') }}" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="descricao" id="descricao" rows="4" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ $evento->descricao }}</textarea>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="profissionais[]" id="profissionais" multiple
                                ...
                                @foreach ($profissionais as $profissional)
                                    <option value="{{ $profissional->id }}" {{ in_array($profissional->id, $evento->profissionais->pluck('id')->toArray()) ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="acolhidos[]" id="acolhidos" multiple
                                ...
                                @foreach ($acolhidos as $acolhido)
                                    <option value="{{ $acolhido->id }}" {{ in_array($acolhido->id, $evento->acolhidos->pluck('id')->toArray()) ? 'selected' : '' }}>
                                        {{ $acolhido->nome }} - {{ $acolhido->cpf }}
                                    </option>
                                @endforeach
                            </select>
                            <small class="text-gray-500">Você pode buscar por nome ou CPF.</small>
                    ...
                    <form method="POST" action="{{ route('eventos.destroy', $evento->id) }}" id="formDelete" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-red-700 hover:bg-red-900 px-6 py-2 rounded text-white">
                            Excluir Evento
                        </button>
                    </form>
                </div>
